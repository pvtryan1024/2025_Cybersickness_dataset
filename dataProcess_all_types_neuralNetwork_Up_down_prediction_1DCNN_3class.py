...
def weighted_sparse_categorical_crossentropy(weights):
    def loss(y_true, y_pred):
        ...
        return tf.reduce_mean(scce * sample_weights)
    return loss

# Example: if "same" dominates (class 1), reduce its weight
# manual_weights = tf.constant([4.0, 0.5, 4.0], dtype=tf.float32)  # [down, same, up]
manual_weights = tf.constant([3.0, 0.5, 3.0], dtype=tf.float32)  # [down, same, up]
loss_fn = weighted_sparse_categorical_crossentropy(manual_weights)

model_cnn = Sequential([
    Conv1D(128, kernel_size=30, activation='relu', padding='same', input_shape=(400, 7)),
    BatchNormalization(),
    Dropout(0.3),

    Conv1D(128, kernel_size=30, activation='relu', padding='same'),
    BatchNormalization(),
    Dropout(0.3),
...
plt.plot(y_pred[sample].squeeze(), label='Predicted')
plt.title("Up/Down Prediction - Sample 0")
plt.legend()
plt.show()


plt.plot(y_test[sample].squeeze())
plt.title("Data Distribution - Sample 0")
plt.show()

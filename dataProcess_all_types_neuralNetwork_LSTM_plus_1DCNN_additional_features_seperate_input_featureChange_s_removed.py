...
for csv_file in csv_files:
    ...
    x_final = np.delete(x_final, 10, axis=1)  # shape: (420, 10)  
    x_final = np.delete(x_final, 9, axis=1)  # shape: (420, 9)  
    print("Final x shape:", x_final.shape)
...
static_input = Input(shape=(2,), name='static_input')         # static input
...
X_ts_train, X_ts_test, X_static_train, X_static_test, y_train, y_test = train_test_split(
    X_ts, X_numeric_scaled, y, test_size=0.2, random_state=42
)
...
#     plt.ylim(0, 20)
#     plt.title("LSTM_Gender_Removed (Sample " + str(i) + ")")
#     plt.legend()
#     plt.show()

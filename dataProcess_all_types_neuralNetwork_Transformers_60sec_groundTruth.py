...
print(y.shape)
downsample_len = 120  # number of time steps to aggregate
num_samples, time_steps, num_features = y.shape

# Compute number of blocks
# Compute number of blocks
num_blocks = int(np.ceil(time_steps / downsample_len))

# Create downsampled y with the same shape
y_downsampled_same_shape = np.zeros_like(y, dtype=float)

for i in range(num_samples):
    for b in range(num_blocks):
        start = b * downsample_len
        end = min((b + 1) * downsample_len, time_steps)
        # Aggregate over the block for all features
        block_value = y[i, start, :]
        y_downsampled_same_shape[i, start:end, :] = block_value

print(y_downsampled_same_shape.shape)  # (427, 420, 1)
...
X_ts_train, X_ts_test, X_static_train, X_static_test, y_train, y_test , y_true_train, y_true_test = train_test_split(
    X_ts, X_static_scaled, y_downsampled_same_shape, y, test_size=0.2, random_state=42
)
...
history = model.fit([X_ts, X_static_scaled], y_downsampled_same_shape, epochs=30, batch_size=16, validation_split=0.2)
...
y_test_true_flat = y_true_test.flatten()
...
smape_value = calculate_smape(y_test_true_flat, y_pred_flat)
...
for i in sampleIndex:
    ...
    plt.ylim(0, 20)
    plt.title("Transformer Comparison Downsampled vs. Ground truth (Sample " + str(i) + ")")
    plt.legend()
    plt.show()


for i in sampleIndex:
    dtw_result = calculate_dtw(y_downsampled_same_shape[i].squeeze(), y_pred1[i].squeeze())
    dtw_result_list.append(dtw_result)
    print("////////////////////"+ str(i)+"//////////////////////////////")
    # print(dtw_result.path)
    print(dtw_result.distance)
    distanceSum += dtw_result.distance
    plt.plot(y_downsampled_same_shape[i].squeeze(), label='True')
    plt.plot(y_pred1[i].squeeze(), label='Predicted')
    plt.ylim(0, 20)
    plt.title("Transformer Comparison_Down Sampled vs Down sampled Truth (Sample " + str(i) + ")")
    plt.legend()
    plt.show()

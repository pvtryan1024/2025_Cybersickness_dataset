...
for csv_file in csv_files:
    ...
    x_cols = list(range(2, 8))  # [0, 2, 3, 4, 5, 6]
...
model_lstm = Sequential([
    # LSTM layers
    LSTM(128, return_sequences=True, input_shape=(400, 6)),
    Dropout(0.2),
    BatchNormalization(),
...
#     plt.ylim(0, 20)
#     plt.title("LSTM time excluded (Sample " + str(i) + ")")
#     plt.legend()
#     plt.show()



y_pred1 = model_lstm.predict(X)
...
for i in sampleIndex:
    ...
    plt.title("LSTM Time Removed (Sample " + str(i) + ")")
    plt.legend()
    plt.show()

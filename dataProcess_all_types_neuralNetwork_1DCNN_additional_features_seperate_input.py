...
# Inputs
ts_input = Input(shape=(420, 8), name='time_series_input')    # dynamic input
static_input = Input(shape=(3,), name='static_input')         # static input

# CNN blocks
x = Conv1D(64, kernel_size=25, activation='relu', padding='same')(ts_input)
x = BatchNormalization()(x)
x = Dropout(0.3)(x)

x = Conv1D(128, kernel_size=25, activation='relu', padding='same')(x)
x = BatchNormalization()(x)
x = Dropout(0.3)(x)
...
# Concatenate static and dynamic streams along feature axis
combined = Concatenate(axis=-1)([x, repeated_static])  # shape: (batch, 420, 128+3)
